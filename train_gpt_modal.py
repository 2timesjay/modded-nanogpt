...
COMMIT_SHA = "d6a7f06e6d2ec388bbd52b1561dd3a64e5e775fa"
SCRIPT_URL = f"https://raw.githubusercontent.com/KellerJordan/modded-nanogpt/{COMMIT_SHA}/train_gpt.py"

image = (
    modal.Image.debian_slim(python_version="3.12.7")
    .pip_install("numpy<3", "tqdm")
    .pip_install(
        "torch==2.7.0.dev20250110+cu124",
        pre=True,
        index_url="https://download.pytorch.org/whl/nightly/cu124",  # tested with torch-2.6.0.dev20241120
    )
    .apt_install("wget")
    .run_commands([f"wget -O {TARGET + 'train_gpt_downloaded.py'} {SCRIPT_URL}"])
...
def train(train_gpt_distributed_code: str):
    import os
    import subprocess

    with open("train_gpt_distributed.py", "w") as file:
        file.write(train_gpt_distributed_code)
    ...
    subprocess.run(
        ["torchrun", "--standalone", f"--nproc_per_node={N_H100}", "train_gpt_distributed.py"]
    )


@app.local_entrypoint()
def main():
    with open("train_gpt_distributed.py", "r") as file:
        train_gpt_distributed_code = file.read()
    get_data.remote()
    train.remote(train_gpt_distributed_code)